...
import spark.jobserver.JobManagerActor.{GetContextData, ContextData}
...
object ContextSupervisor {
  ...
  case class GetSparkContextData(name: String)
  ...
  case class SparkContextData(name: String, appId: String, url: Option[String])
}
  ...
  def wrappedReceive: Receive = {
    ...
    case GetSparkContextData(name) =>
      contexts.get(name) match {
        case Some((actor, _)) =>
          val future = (actor ? GetContextData)(contextTimeout.seconds)
